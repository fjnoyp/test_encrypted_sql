...
import 'package:test_encrypted_sql/db_common/connect_io.dart';
import 'package:test_encrypted_sql/queued_item_store/drift/android_support.dart';
...
class DriftQueuedItemStore extends _$DriftQueuedItemStore
  ...
  factory DriftQueuedItemStore(String storagePath) {
    // Not calling this results in:
    // Unhandled Exception: Invalid argument(s): Failed to load dynamic library '/data/data/com.example.test_encrypted_sql/lib/libsqlite3.so'
    setupSqlCipher();

    final dbPath = p.join(storagePath, 'com.amplify.test.sqlite');
    final databaseImpl = NativeDatabase(File(dbPath), setup: (rawDb) {
    ...
    return DriftQueuedItemStore._(databaseImpl);
  }
